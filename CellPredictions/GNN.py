...
class HierarchicalCrystalGNN(nn.Module):
    def __init__(self, node_feat_dim=2, hidden_dim=128, num_hall_classes=532):
        super().__init__()
    ...
    def forward(self, x, edge_index, batch, condition_hall_idx=None):
        ...
        cell_pred = self.cell_regressor(combined_emb)

        return hall_logits, cell_pred
...
def train(model, loader, optimizer, device, alpha=1.0):
    ...
    for batch in loader:
        ...
        hall_logits, cell_pred = model(batch.x, batch.edge_index, batch.batch, condition_hall_idx=batch.hall_y)
...
def evaluate(model, loader, device, alpha=1.0):
    ...
    return total_loss / len(loader), total_hall_acc / len(loader.dataset), total_cell_mae / len(loader.dataset)
...
def predict_top_k(model, smiles_string, device, k=5):
    ...
    with torch.no_grad():
        ...
        for prob, hall_idx in zip(top_k_probs, top_k_indices):
            ...
            combined_emb = torch.cat([mol_emb, hall_emb], dim=1)
            cell_pred = model.cell_regressor(combined_emb)[0]
...
if __name__ == "__main__":
    ...
    # 1. Create dummy data if CSV doesn't exist (For testing the script)
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch:03d} | Train Loss: {train_loss:.4f}, Hall Acc: {train_acc:.4f} | "
              f"Val Loss: {val_loss:.4f}, Hall Acc: {val_acc:.4f}, Cell MAE: {val_mae:.4f}")

    # 5. Inference Demonstration
    print("\nTraining Complete! Testing Inference...")
    test_smiles = "CC(=O)OC1=CC=CC=C1C(=O)O" # Aspirin
    print(f"Predicting top 3 solutions for {test_smiles}:")
    predictions = predict_top_k(model, test_smiles, device, k=3)

    for i, pred in enumerate(predictions, 1):
        print(f"Rank {i}: Hall {pred['hall_number']} (Prob: {pred['probability']:.3f}) | "
